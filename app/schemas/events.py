...
Convención de nombres:
  - eventType: UPPER_SNAKE_CASE  (ej: ORDER_CREATED, SHIPMENT_DELIVERED)
  - campos de payload: camelCase estricto
"""
...
class EventEnvelope(BaseModel):
    ...
    eventType: str = Field(..., description="Tipo de evento (ej: ORDER_CREATED)")
...
class OrderCreatedPayload(BaseModel):
    """Payload del evento ORDER_CREATED emitido por el Grupo 5 (Pedidos)."""
...
class PaymentApprovedPayload(BaseModel):
    """Payload del evento PAYMENT_APPROVED emitido por el Grupo 8 (Pagos)."""
...
class InventoryShortagePayload(BaseModel):
    """Payload del evento INVENTORY_SHORTAGE emitido por el Grupo 4 (Inventario)."""
...
class ShipmentDeliveredPayload(BaseModel):
    """Payload del evento SHIPMENT_DELIVERED emitido por el Grupo 6 (Despacho)."""

    shipmentId: str = Field(..., description="Identificador del envío")
    orderId: str = Field(..., description="Pedido asociado al envío")
    deliveredAt: datetime = Field(..., description="Fecha y hora de entrega efectiva")
